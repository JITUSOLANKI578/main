...
const promotion = async (req, res) => {
   ...
   const [user] = await connection.query('SELECT `phone`, `code`,`invite`, `roses_f`, `roses_f1`, `roses_today`,`f1_today2`, `f_total`  FROM users WHERE `token` = ? ', [auth]);
    ...
    return res.status(200).json({
        ...
        invite: {
            ...
            f1_today2: userInfo.f1_today2,
            f_total: userInfo.f_total,
            roses_all: rosesAdd,
...
const myTeam = async (req, res) => {
    ...
    const [user] = await connection.query('SELECT `phone`, `code`,`invite`,`yrec` FROM users WHERE `token` = ? ', [auth]);
...
const listMyTeam = async (req, res) => {
    ...
    let userInfo = user[0];
    const [f1] = await connection.query('SELECT `id_user`,`yrec` ,`phone`, `code`, `invite`,`roses_f`, `rank`, `name_user`,`status`,`total_money`, `time` FROM users WHERE `invite` = ? ORDER BY id DESC', [userInfo.code]);
    ...
    async function fetchUserDataByCode(code, depth = 1) {
        ...
        const [userData] = await connection.query('SELECT `id_user`,`yrec`, `name_user`, `phone`, `code`, `invite`, `rank`, `total_money` FROM users WHERE `invite` = ?', [code]);
