...
const transrec = async (req, res) => {
    return res.render("member/about/transrec.ejs");
}
...
const getTransRecord = async (req, res) => {
    const auth = req.cookies.auth;

    const [rows1] = await connection.query(`SELECT * FROM users WHERE token = ?`, [auth]);
    ...
    const [getPhone] = await connection.query(
        `SELECT * FROM transcations WHERE phone = ? ORDER BY tid DESC`,
        [rowstr1.phone]
    );


    console.log("asdasdasd : " + [rows1.phone])
    return res.status(200).json({
        message: 'Success',
        status: true,
        data: {
